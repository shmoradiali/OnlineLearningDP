...
def oracle_ftrl(L, rate):
    ...
    x = cp.Variable(dim)
    a = np.ones(dim)
    objective = cp.Minimize(rate * L @ x - a @ cp.entr(x))
    constraints = [x >= 0, a @ x <= 1.01, a @ x >= 0.99]
    prob = cp.Problem(objective, constraints)
    try:
        prob.solve()
    except:
...
if __name__ == "__main__":
    ...
    fig.savefig(os.path.join("plots", f"prediction_from_experts_advice_d={dim}.pdf"), bbox_inches='tight')
